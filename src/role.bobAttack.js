...
module.exports = {
    ...
    run: function (creep) {
        ...
        return;
        var AttackTarget_1 = Game.flags.AttackTarget_1;
        var AttackTarget_2 = Game.flags.AttackTarget_2;
        var AttackTarget_3 = Game.flags.AttackTarget_3;


        // var towerTargetLocation = new RoomPosition(42, 27, "E26N3")
        var look = AttackTarget_1.room.lookAt(AttackTarget_1)
        //console.log('<font color = "yellow">[' + fileName + 'line:' + util.LineNumber() + '] room[' + creep.room.name + '] JSON.stringify (look) is ' + JSON.stringify(look) + '</>');
        console.log('<font color = "yellow">[' + fileName + 'line:' + util.LineNumber() + '] room[' + creep.room.name + '] JSON.stringify (look[0].structure.pos) is ' + JSON.stringify(look[0].structure.pos) + '</>');

        if (look[0].structure != undefined) {
            var attackStatus = creep.attack(look[0].structure);
            if (attackStatus != 0) {
                var travelStatus = creep.travelTo(look[0].structure)
            }
            return;
        }

        var look = AttackTarget_2.room.lookAt(AttackTarget_2)
        if (look[0].structure != undefined) {
            var attackStatus = creep.attack(look[0].structure);
            if (attackStatus != 0) {
                var travelStatus = creep.travelTo(look[0].structure)
        ...
        var look = AttackTarget_3.room.lookAt(AttackTarget_3)
        if (look[0].structure != undefined) {
            var attackStatus = creep.attack(look[0].structure);
            if (attackStatus != 0) {
                var travelStatus = creep.travelTo(look[0].structure)
                console.log('<font color = "yellow">[' + fileName + 'line:' + util.LineNumber() + '] travelStatus is ' + travelStatus + '</>');
            }
            return;
        }


        creep.attack(look[0].structure);
        return;
        // var attackStructureTargets = [];

        // look.forEach(function (lookObject) {
        //     if (lookObject.type == LOOK_CREEPS
        //         || lookObject.type == LOOK_STRUCTURES
        //     ) {
        //         attackStructureTargets.push(lookObject.structure);
        //         //  tower.attack(lookObject.creep);
        //     }
        // });

        // var look = tower.room.lookAt(AttackTarget_2)
        // look.forEach(function (lookObject) {
        //     if (lookObject.type == LOOK_CREEPS
        //         || lookObject.type == LOOK_STRUCTURES
        //     ) {
        //         attackStructureTargets.push(lookObject);
        //         //  tower.attack(lookObject.creep);
        //     }
        // });

        // for (let index = 0; index < attackStructureTargets.length; index++) {
        //     const attackTarget = attackStructureTargets[index];
        //     //  console.log('<font color = "yellow">[' + fileName + 'line:' + util.LineNumber() + '] room[' + creep.room.name + '] attackTarget.structureType is ' + JSON.stringify(attackTarget.structureType) + '</>');
        //     while (attackTarget != undefined) {
        //         var attackStatus = creep.attack(attackTarget)
        //         if (attackStatus != 0) {
        ...
        // if (AttackTarget_1 != undefined) {
        //     var attackStatus = creep.attack(AttackTarget_1)
        //     console.log('<font color = "yellow">[' + fileName + 'line:' + util.LineNumber() + '] room[' + creep.room.name + '] attackStatus is ' + attackStatus + '</>');
        //     if (attackStatus != 0) {
        //         creep.travelTo(AttackTarget_1);
        //     }
        //     return;
        // }

        // if (AttackTarget_2 != undefined) {
        //     var attackStatus = creep.attack(AttackTarget_2)
        //     if (attackStatus != 0) {
        //         creep.travelTo(AttackTarget_2);
        //     }
        //     return;
        // }
...
function goFromFlagToFlag(creep) {
    ...
    if (creep.pos.isEqualTo(targetFlag.pos)) {
        ...
        if (creep.memory.targetFlagIndex == 4) {
            creep.memory.targetFlagIndex = 0;
        }
    }
}

function attackStructuresAtFlag(creep, flag) {
    var look = flag.room.lookAt(flag)
    if (look[0].structure != undefined) {
        var attackStatus = creep.attack(look[0].structure);
        if (attackStatus != 0) {
            var travelStatus = creep.travelTo(look[0].structure)
