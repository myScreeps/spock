...
module.exports = function () {
    ...
    StructureSpawn.prototype.createClaimer = function (target) {
        ...
        return this.createCreep([CLAIM, MOVE, WORK, WORK, WORK, WORK, CARRY, CARRY, CARRY, CARRY, MOVE, MOVE, MOVE, MOVE], undefined, { role: 'claimer', target: target });
    };

    StructureSpawn.prototype.createBob = function () {
        // console.log("Creating a claimer");
        var creepName = "bob_" + Game.time;
        return this.createCreep([TOUGH, TOUGH, TOUGH, TOUGH, TOUGH, TOUGH, TOUGH, TOUGH, TOUGH, TOUGH,
    ...
    StructureSpawn.prototype.createBobA = function () {
        ...
        return this.createCreep([MOVE, ATTACK], creepName, { role: "bobA" });
        //  return this.createCreep([TOUGH,TOUGH,TOUGH, TOUGH, TOUGH,MOVE,MOVE,MOVE,MOVE,ATTACK], creepName, { role: "bobA" });
    };
    ...
    StructureSpawn.prototype.createMiner = function (spawn, sourceId) {
        ...
        //     energy = spawn.memory.EnergyManagement.minerEnergy;
        // }
        ...
        // #HACK WARNING
        if (spawn.room.name == "W8N36"
            || spawn.room.name == "W8N35"
            || spawn.room.name == "W9N34") {
            body = [WORK, WORK, WORK, WORK, WORK, WORK, WORK, WORK, MOVE, CARRY, CARRY, MOVE, MOVE, MOVE];
            return nearestSpawnToSource.createCreep(body, 'miner_' + Game.time,
            ...
            return nearestSpawnToSource(body, 'miner_' + Game.time,
                { role: 'miner', orginalRole: 'miner', sourceId: sourceId, home: spawn.room.name, target: spawn.room.name, gameStartTick: Game.time, respawnOffSet: body.length * 3 });
            // } else if (energy <= 700) {
            ...
            var body = [WORK, WORK, WORK, WORK, WORK, MOVE, MOVE, CARRY, CARRY];
            return nearestSpawnToSource(body, 'miner_' + Game.time,
                { role: 'miner', orginalRole: 'miner', sourceId: sourceId, home: spawn.room.name, target: spawn.room.name, gameStartTick: Game.time, respawnOffSet: body.length * 3 });
        }
        else if (energy <= 850) {
            var body = [WORK, WORK, WORK, WORK, WORK, WORK, MOVE, MOVE, MOVE, CARRY, CARRY];
            return nearestSpawnToSource(body, 'miner_' + Game.time,
                { role: 'miner', orginalRole: 'miner', sourceId: sourceId, home: spawn.room.name, target: spawn.room.name, gameStartTick: Game.time, respawnOffSet: body.length * 3 });
        }

        else if (energy <= 1000) {
            var body = [WORK, WORK, WORK, WORK, WORK, WORK, WORK, MOVE, MOVE, MOVE, MOVE, CARRY, CARRY];
            return nearestSpawnToSource.createCreep(body, 'miner_' + Game.time,
                { role: 'miner', orginalRole: 'miner', sourceId: sourceId, home: spawn.room.name, target: spawn.room.name, gameStartTick: Game.time, respawnOffSet: body.length * 3 });
        }
    ...
    StructureSpawn.prototype.createFlagToFlagHarvester2 = function (SpawnObj, roleName, energy, _flagSource, _flagContainer) {
        ...
        //    body.push(MOVE);
        //}
        //  console.log('<font color = "yellow">[' + fileName + 'line:' + util.LineNumber() + '] room[' + this.room.name + '] SpawnObj.memory.energyFlagToFlagHarvester2 is ' + this.memory.energyFlagToFlagHarvester2 +'</>');
        //  console.log('<font color = "yellow">[' + fileName + 'line:' + util.LineNumber() + '] room[' + this.room.name + '] energy is ' + energy +'</>');
        var body = [];
        var numberOfParts = Math.floor(energy / 100);
        //  console.log('<font color = "yellow">[' + fileName + 'line:' + util.LineNumber() + '] room[' + this.room.name + '] numberOfParts is ' + numberOfParts +'</>');
        for (let index = 0; index < numberOfParts; index++) {
            //  const element = array[index];
            body.push(CARRY);
            body.push(MOVE);
        }
        //  console.log('<font color = "yellow">[' + fileName + 'line:' + util.LineNumber() + '] room[' + this.room.name + '] JSON.stringify(body) is ' + JSON.stringify(body) +'</>');
        //body.push(WORK);
        // body.push(WORK);
        // body.push(WORK);
        //   body.push(CARRY);
    ...
    StructureSpawn.prototype.renewCreepsNextToSpawn = function (spawn, ticksToLive) {
        ...
        if (target != undefined) {
            ...
            renewStatus = spawn.renewCreep(target);
        }
        return renewStatus;
    };
};
